...
<html lang="en">
    <head>
        ...
        <style>
			...
			#tt {position: absolute; top:140px; left:540px; color:#C00}
        </style>
    </head>
	...
	if(isset($_SESSION["userName"]))
	{
		...
		//b1:mo ket noi:
			include("../ConnectDb/open.php");
		//b2:thao tac lay admin ve
		$sql="select * from tbladmin where userName='$id'";
		$result=mysqli_query($con,$sql);
		$id=mysqli_fetch_array($result);
		?>
        <form action="capnhatttadminprocess.php" enctype="multipart/form-data" method="post">
        	<table>
        		<div id="tt"><h3>Cập nhật thông tin</h3></div>
              <tr>
        		<th>ID:<input type="text" name="txtma" value="<?php echo($id["maTK"]);?>" readonly="readonly"/><br /></th>
              </tr>
              <tr>
            	<th>Tên đăng nhập:<input type="text" name="txtuser" value="<?php echo($id["userName"]);?>" /><br /></th>
              </tr>
           		<tr>
        		<th>Password:<input type="text" name="txtpass" value="<?php echo($id["passWord"]);?>" /><br /></th>
              </tr>
              <tr>
       			<th>Ảnh:<input type="file" name="txtanh" value="<?php echo($id["anh"]);?>" /><br /></th>
              </tr>
              <tr>
        		<th>Tên admin:<input type="text" name="txtten" value="<?php echo($id["ten"]);?>"/><br /></th>
              </tr>
              <tr>
        		<th>Ngày sinh:<input type="date" name="txtdob" value="<?php echo($id["ngaySinh"]);?>"/><br /></th>
              </tr>
              <tr>
        		<th>Giới tinh:<input type="radio" name="rdbGt" value="1" <?php if (($id["gioiTinh"])==1) { ?> checked="checked" <?php }?>/>Nam
