...
<html lang="en">
    <head>
        ...
        <style>
			...
			#tt {position: absolute; top:140px; left:500px; color:#C00}
        </style>
    </head>
	...
	if(isset($_GET["ma"]))
	{
		...
		$sql="select * from danhmuc where ma='$dm'";
		$result=mysqli_query($con,$sql);
		$dm=mysqli_fetch_array($result);
		?>
        <form action="suadmprocess.php">
        	<table>
        		<div id="tt"><h3>Sửa thông tin danh mục</h3></div>
                <tr>
        			<th>Mã danh mục:<input type="text" name="txtma" value="<?php echo($dm["ma"]);?>" readonly="readonly"/><br /></th>
                 </tr>
                 <tr>
            		<th>Tên danh mục:<input type="text" name="txtten" value="<?php echo($dm["ten"]);?>"/><br /></th>
                 </tr>
   			<?php
