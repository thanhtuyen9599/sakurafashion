...
<html lang="en">
    <head>
        ...
        <style>
			...
			#tt {position: absolute; top:130px; left:480px; color:#C00}
        </style>
    </head>
	...
	{
		$loaisp=$_GET["maLoaisp"];
		//b1:mo ket noi:
			include("../ConnectDb/open.php");
		//b2:thao tac lay loai sp ve
		$sql="select * from tblloaisp where maLoaisp='$loaisp'";
		$result=mysqli_query($con,$sql);
		$loaisp=mysqli_fetch_array($result);
		?>
        <form action="sualoaispprocess.php">
        	<table>
        		<div id="tt"><h3>Sửa thông tin loại sản phẩm</h3></div>
                <tr>
        			<th>Mã loại SP:<input type="text" name="txtma" value="<?php echo($loaisp["maLoaisp"]);?>" readonly="readonly"/><br /></th>
                </tr>
                <tr>
            		<th>Tên loại SP:<input type="text" name="txtten" value="<?php echo($loaisp["tenLoaisp"]);?>"/><br /></th>
                </tr>
                <tr>
            		<th>Danh mục:<select name="danhmuc">
   			<?php
